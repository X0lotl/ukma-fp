...
import           Text.ParserCombinators.Parsec (Parser, spaces, string, eof, many1, digit, oneOf, parse)
...
addVar x xs = if x `elem` xs then xs else x : xs
...
reduce (Abs y t) x s
  | x == y = Abs y t
  | y `notElem` freeVars s = Abs y (reduce t x s)
  | otherwise =
...
evalStep (Var x) ctx = valFromCtx ctx x <|> Nothing
evalStep (App (Abs x b) t) _ = Just (reduce b x t)
evalStep (App t1 t2) ctx =
  let r1 = evalStep t1 ctx
   in case r1 of
        Nothing    -> (Just . App t1) =<< evalStep t2 ctx
        (Just tr1) -> Just (App tr1 t2)
evalStep (Abs x t) ctx = Just . Abs x =<< evalStep t (deleteSyn x ctx)
...
eval mx t ctx =
  let step (cnt, Just t1) = (cnt + 1, evalStep t1 ctx <|> Just t1)
      step t1 = t1
...
canEval (Var x) ctx         = Data.Maybe.isJust (valFromCtx ctx x)
...
newVar fvs nm = until (`notElem` fvs) next (next nm) -- flip elem fvs
  where
    next n@(c : _) | c == '9' = '0' : n
