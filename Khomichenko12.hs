...
isTerminal (_, sts, _) s = s `elem` sts
...
closure naut ss =
  ...
  where
    step (acc, bs) =
      let newSts = sort (setStep naut [b | b <- bs, b `notElem` acc] Eps)
...
accepts naut@(is, _, _) wd =
  let ssn = foldl step (closure naut [is]) wd
        where
          step bs ch = closure naut (setStep naut bs (C ch))
